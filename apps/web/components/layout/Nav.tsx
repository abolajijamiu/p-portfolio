          ...
          <nav className="flex-1 overflow-y-auto px-5 py-6 space-y-1">
            {LINKS.map(({ label, href }) => (
              <Link
                key={href}
                href={href}
                className="flex items-center py-3 text-base font-medium text-ink/70 hover:text-ink transition-[color] duration-150 border-b border-border/50 last:border-0"
              >
                {label}
              </Link>
            ))}
