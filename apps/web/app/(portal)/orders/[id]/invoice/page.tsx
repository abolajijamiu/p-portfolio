...
export default function InvoicePage() {
  ...
  useEffect(() => {
    api.get<OrderDetail>(`/service-orders/${id}`)
      .then(setData)
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-3">
        ...
        <Link href={`/orders/${id}`} className="text-xs text-brand hover:underline">← Back to order</Link>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <div className="print:hidden bg-surface border-b border-border px-6 py-3 flex items-center justify-between gap-4">
        <Link href={`/orders/${id}`} className="text-xs text-muted hover:text-brand transition-colors">
          ← Back to order
        </Link>
        <button
