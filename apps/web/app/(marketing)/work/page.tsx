...
import { WORK, CATEGORY_LABEL, ECOMMERCE_CATEGORIES, type WorkCategory } from '@/lib/content/work'
...
const CATEGORY_FILTERS: { label: string; value: WorkCategory | 'all' | 'ecommerce' }[] = [
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border rounded-xl overflow-hidden">
          {featured.map((item) => (
            ...
            >
              {/* Visual */}
              <div
                className={`w-full aspect-[16/8] rounded-lg mb-6 relative overflow-hidden ${item.accent} flex items-end`}
              >
                ...
                {item.proof.slice(0, 3).map((p) => (
                  <div key={p.label}>
                    <p className="text-base font-semibold text-ink tracking-tight">
                      {p.direction === 'up' ? '' : p.direction === 'down' ? '' : ''}{p.metric}
                    </p>
        ...
        <div className="divide-y divide-border">
          {rest.map((item) => (
            ...
            >
              {/* Color swatch */}
              <div
                className={`w-full md:w-48 shrink-0 aspect-[3/2] md:aspect-auto md:h-24 rounded-lg ${item.accent}`}
              />
